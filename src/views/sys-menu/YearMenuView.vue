<template>
  <div id="app">
    ...
    <component :is="currentMenuView"></component>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getRootItems() {
      ...
      const yearMenuId = 100;
      let url = `http://localhost:8081/treeMenu/menus/${yearMenuId}`;
    ...
    async getChildren(menuId) {
      let url = `http://localhost:8081/treeMenu/${menuId}`;
    ...
    async handleSearch() {
      ...
      const yearMenuId = 100;
      let url = `http://localhost:8081/treeMenu/menus/search/${yearMenuId}/${this.searchParam}`;
      if (this.searchParam) {
        const res = await axios.get(url);
        const data = res.data.data.menus;
        this.treeData = data
